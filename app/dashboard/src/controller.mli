...
(** Render-ready projection the browser layer reads. Every field is plain
    ...
    The panes are grouped into one submodule each so a new pane is a
    localised addition (a new submodule + one field on {!t}) rather than a
    scatter of fields across a flat record. *)
module Display : sig
...
(** The default snapshot capacity (~60s of history) the dashboard uses when
    it has no reason to pick another. *)
