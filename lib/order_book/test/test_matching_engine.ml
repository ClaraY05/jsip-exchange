...
let%expect_test "book: rejects out-of-range ids at the boundary" =
  (* The harness trades 3 symbols (ids 0..2), so the books array has
     length 3. Id 3 is exactly one past the end — the case an off-by-one
     bounds check ([> length] instead of [>= length]) would let through and
     crash on. A negative id is the other edge. Both must resolve to [None],
     since the engine cannot trust an id a client sends. *)
  let t = Harness.create () in
  let engine = Harness.engine t in
  ...
  [%test_result: _ option]
    ...
    ~expect:None;
  [%test_result: _ option]
    (Matching_engine.book engine negative)
    ~expect:None
;;
