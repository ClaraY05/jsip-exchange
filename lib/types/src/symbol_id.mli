(** A compact integer handle for a trading symbol, carried on the wire.
    ...
    that int ([to_string] is [Int.to_string], sexps show the number). Recovering
    the human-readable name from an id is a {e consumer-side} concern — the
    Phase 2 symbol directory — and is intentionally NOT built into this type, so
    [lib/types] stays free of any name registry. Validating that an id is in
    range for a particular exchange is the {e server's} job, not this type's. *)
